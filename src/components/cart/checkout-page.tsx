...
export function CheckoutPageContent() {
  const { lines, subtotal, currency, hydrated } = useStore();
  const [code, setCode] = useState('');
  const shipping = subtotal >= siteConfig.freeShippingThreshold || subtotal === 0 ? 0 : 6.95;
              ...
              <fieldset>
                ...
                <div className="rounded-xl border border-line bg-canvas p-4 text-sm text-ink-muted">
                  <p className="flex items-center gap-2">
                    <Lock size={14} /> This is a template demo — connect Shopify Checkout or your payment
                    provider to process real orders.
                  </p>
                </div>
              </fieldset>

              <button className="btn-accent w-full py-4 text-base">
                <Lock size={16} /> Pay {formatMoney(total, currency)}
              </button>
          ...
          <div className="mx-auto max-w-md lg:sticky lg:top-12">
            ...
            <div className="mt-6 flex gap-2">
              <div className="relative flex-1">
                ...
                <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="Gift card or discount code" className="input pl-9" />
              </div>
